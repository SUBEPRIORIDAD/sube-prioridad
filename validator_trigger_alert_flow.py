...
Regla central:
    ...
    No revela diagnóstico.
    No revela CUD.
    No revela DNI.
    No revela identidad civil.
    No obliga al chofer.
...
Este módulo no acredita beneficios, no calcula tarifas y no interactúa con
validadores reales.
"""
...
def create_demo_user_alert_preferences(
    ...
    return UserAlertPreferences(
        ...
        allow_passive_internal_alert=allow_passive_internal_alert,
        allow_discreet_visual_alert=allow_discreet_visual_alert,
        allow_visible_light_alert=allow_visible_light_alert,
        allow_luminous_signal=allow_luminous_signal,
        allow_station_operator_notification=allow_station_operator_notification,
        allow_transport_operator_notification=allow_transport_operator_notification,
        allow_silent_log=allow_silent_log,
    )
...
def evaluate_validator_trigger_alert(
    request: ValidatorTriggerAlertRequest,
) -> ValidatorTriggerAlertResult:
    """
    Evalúa si corresponde disparar una alerta demo posterior a validación.
    """
    ...
    if hard_risk_flags:
        return _result(
            ...
            reason=(
                "No se disparó alerta porque faltan condiciones mínimas: atributo activo, "
                "necesidad previamente acreditada, validación paga o contexto compatible."
            ),
        )
    ...
    if not selected_channels:
        return _result(
            ...
            audit_flags=audit_flags,
            reason=(
                "No se disparó alerta porque las preferencias del usuario no habilitan "
                "canales activos para este contexto."
            ),
        )
    ...
    return _result(
        ...
        audit_flags=audit_flags,
        reason=(
            "Alerta demo posterior a validación generada según preferencias del usuario "
            "y sin exponer datos sensibles."
        ),
    )


def simulate_validator_trigger_alert(
    request: Optional[ValidatorTriggerAlertRequest] = None,
) -> ValidatorTriggerAlertResult:
    """
    Alias estable usado por demos y tests.
    """
    return evaluate_validator_trigger_alert(
        request or create_demo_validator_trigger_alert_request()
    )
...
def result_to_dict(result: ValidatorTriggerAlertResult) -> Dict[str, Any]:
    return {
        ...
        "station_or_operator_notification_requested": (
            result.station_or_operator_notification_requested
        ),
        "exposes_sensitive_data": result.exposes_sensitive_data,
...
def _hard_risk_flags(request: ValidatorTriggerAlertRequest) -> List[str]:
    flags: List[str] = []
    priority = request.priority_attribute
    event = request.validation_event

    if _looks_like_free_text(priority.priority_user_token):
        flags.append("priority_user_token_looks_like_free_text")

    if _looks_like_free_text(priority.priority_attribute_token):
        flags.append("priority_attribute_token_looks_like_free_text")

    if _looks_like_free_text(event.validation_event_demo_id):
        flags.append("validation_event_id_looks_like_free_text")

    if not priority.active:
        flags.append("priority_attribute_not_active")

    if not priority.previously_accredited_need:
        flags.append("priority_need_not_previously_accredited")

    if not event.validation_paid:
        flags.append("validation_payment_not_confirmed")

    if event.country != "AR":
        flags.append("outside_argentina_context")

    if event.validation_point_type == ValidationPointType.UNKNOWN:
        flags.append("unknown_validation_point_type")
...
def _audit_flags(request: ValidatorTriggerAlertRequest) -> List[str]:
    ...
    if event.validation_point_type == ValidationPointType.VEHICLE_VALIDATOR:
        flags.append("vehicle_validator_context")

    if event.validation_point_type in {
        ValidationPointType.STATION_TURNSTILE,
        ValidationPointType.STATION_ACCESS_GATE,
    }:
        flags.append("station_or_turnstile_context")

    if event.transport_mode in {
        TransportMode.TRAIN,
        TransportMode.SUBWAY,
        TransportMode.COASTAL_TRAIN_OR_ONBOARD_VALIDATOR,
    }:
        flags.append("rail_or_subway_context")
...
def _select_channels(request: ValidatorTriggerAlertRequest) -> List[AlertChannel]:
    ...
    if preferences.allow_silent_log:
        channels.append(AlertChannel.SILENT_LOG)

    if preferences.allow_passive_internal_alert and preferences.preference_mode in {
        AlertPreferenceMode.PASSIVE,
        AlertPreferenceMode.DISCREET,
        AlertPreferenceMode.VISIBLE,
        AlertPreferenceMode.LUMINOUS,
    }:
        channels.append(AlertChannel.PASSIVE_INTERNAL)

    if preferences.allow_discreet_visual_alert and preferences.preference_mode in {
        AlertPreferenceMode.DISCREET,
        AlertPreferenceMode.VISIBLE,
        AlertPreferenceMode.LUMINOUS,
    }:
        channels.append(AlertChannel.DISCREET_VISUAL)

    if preferences.allow_visible_light_alert and preferences.preference_mode in {
        AlertPreferenceMode.VISIBLE,
        AlertPreferenceMode.LUMINOUS,
    }:
        channels.append(AlertChannel.VISIBLE_LIGHT)

    if preferences.allow_luminous_signal and preferences.preference_mode == AlertPreferenceMode.LUMINOUS:
        channels.append(AlertChannel.LUMINOUS_SIGNAL)

    if (
        preferences.allow_station_operator_notification
        and event.validation_point_type
        in {
            ValidationPointType.STATION_TURNSTILE,
            ValidationPointType.STATION_ACCESS_GATE,
            ValidationPointType.TERMINAL_VALIDATOR,
        }
    ):
        channels.append(AlertChannel.STATION_OPERATOR_DEVICE)

    if preferences.allow_transport_operator_notification:
        channels.append(AlertChannel.TRANSPORT_OPERATOR_DEVICE)

    if preferences.preference_mode == AlertPreferenceMode.SILENT:
        return [AlertChannel.SILENT_LOG] if preferences.allow_silent_log else []
...
def _result(
    ...
    station_or_operator_notification_requested = any(
        channel
        in {
            AlertChannel.STATION_OPERATOR_DEVICE,
            AlertChannel.TRANSPORT_OPERATOR_DEVICE,
        }
    ...
    return ValidatorTriggerAlertResult(
        ...
        status=status,
        selected_channels=selected_channels,
        alert_triggered=status == TriggerStatus.TRIGGERED,
        station_or_operator_notification_requested=station_or_operator_notification_requested,
        exposes_sensitive_data=False,
        driver_burden=(
            "El chofer no recibe diagnóstico, no verifica datos personales, no decide "
            "prioridad y no asume una obligación nueva."
        ),
        hard_risk_flags=hard_risk_flags,
        audit_flags=audit_flags,
        reason=reason,
        privacy_notice=(
            "La alerta demo sólo indica una preferencia de asistencia previamente "
            "configurada. No expone DNI, nombre, diagnóstico, CUD, certificado médico "
            "ni historia clínica."
        ),
        legal_scope_notice=(
            "La alerta es conceptual y complementaria. No reemplaza derechos vigentes, "
            "no crea privilegios, no modifica obligaciones legales y no integra sistemas "
            "reales de SUBE o Red SUBE."
        ),
        warnings=[
            "Alerta posterior a validación conceptual y demostrativa.",
            ...
            "Sin obligación nueva para el chofer.",
            "La preferencia del usuario controla el modo de alerta.",
        ],
