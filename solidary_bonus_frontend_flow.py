...
Finalidad:
    Representar el frontend mínimo, voluntario y controlado por el usuario
    SUBE Prioridad antes de pasar al flujo de blindaje antifraude.

Regla central:
    El Bono Solidario sólo puede ser iniciado por el usuario SUBE Prioridad.
    El colaborador no puede reclamarlo unilateralmente.
    El chofer no interviene.
    El sistema no premia la cesión de asientos prioritarios legales.
"""
...
class PriorityAccountFrontendContext:
    """
    Contexto demostrativo de cuenta SUBE del usuario prioritario.
...
class SolidaryFrontendTransportContext:
    """
    Contexto mínimo demostrativo del viaje.
...
class CandidateCollaborator:
    """
    Posible destinatario demostrativo del Bono Solidario.
...
def create_demo_priority_account_frontend_context(
    ...
    """
    Crea contexto demostrativo de cuenta SUBE para el frontend.
    """
    required_values = {
...
def create_demo_solidary_frontend_transport_context(
    ...
    """
    Crea contexto demostrativo de transporte para el frontend.
    """
...
def create_demo_candidate_collaborator(
    ...
    """
    Crea un destinatario demostrativo del eventual Bono Solidario.
    """
...
def create_demo_solidary_bonus_frontend_request(
    ...
    """
    Crea una solicitud demostrativa iniciada desde frontend.
    """
    ...
    return SolidaryBonusFrontendRequest(
        ...
        priority_account_context=priority_account_context
        or create_demo_priority_account_frontend_context(),
        transport_context=transport_context
        or create_demo_solidary_frontend_transport_context(),
        candidate_collaborator=candidate_collaborator
        or create_demo_candidate_collaborator(),
...
def evaluate_solidary_bonus_frontend_request(
    ...
    """
    Evalúa si el frontend puede preparar el handoff al flujo de seguridad.
    """
    assert_no_prohibited_fields(
        {
    ...
    return SolidaryBonusFrontendResult(
        ...
        risk_flags=[],
        reason=(
            "El frontend conceptual está listo para remitir la solicitud al flujo "
            "de seguridad. Todavía no acredita Bono Solidario; sólo prepara el "
            "handoff demostrativo."
        ),
...
def run_demo() -> Dict[str, Any]:
    """
    Ejecuta una demostración estable del frontend de Bono Solidario.
    """
...
def _requires_review_only(risk_flags: List[str]) -> bool:
    review_only_flags = {
        "demo_scope_acknowledgement_required",
    }
...
def _rejected_result(
    ...
    return SolidaryBonusFrontendResult(
        ...
        version=FLOW_VERSION,
        demo_mode=DEMO_MODE,
        status=FrontendDecisionStatus.REJECTED,
        frontend_ready=False,
        handoff_to_security_flow_enabled=False,
        solidary_point_demo=0,
        risk_flags=risk_flags,
        reason=(
            "El frontend conceptual no puede preparar el handoff al flujo de "
            "seguridad porque la solicitud no supera los filtros mínimos."
        ),
        frontend_channel=request.frontend_channel,
        recipient_selection_method=request.candidate_collaborator.selection_method,
        security_flow_payload_demo={},
        priority_user_control=_priority_user_control_notice(),
        collaborator_limit=_collaborator_limit_notice(),
        validation_context_notice=_validation_context_notice(),
        privacy_notice=_privacy_notice(),
...
def _needs_review_result(
    ...
    return SolidaryBonusFrontendResult(
        ...
        version=FLOW_VERSION,
        demo_mode=DEMO_MODE,
        status=FrontendDecisionStatus.NEEDS_REVIEW,
        frontend_ready=False,
        handoff_to_security_flow_enabled=False,
        solidary_point_demo=0,
        risk_flags=risk_flags,
        reason=(
            "La solicitud conceptual requiere revisión o una confirmación adicional "
            "antes de pasar al flujo de seguridad."
        ),
        frontend_channel=request.frontend_channel,
        recipient_selection_method=request.candidate_collaborator.selection_method,
        security_flow_payload_demo={},
...
def _security_flow_payload_demo(
    request: SolidaryBonusFrontendRequest,
) -> Dict[str, Any]:
    """
    Payload mínimo para el flujo de seguridad posterior.
...
def _priority_user_control_notice() -> str:
    return (
        "El usuario SUBE Prioridad inicia y confirma el envío conceptual. "
        "Sin su confirmación no hay handoff al flujo de seguridad."
    )
...
def _collaborator_limit_notice() -> str:
    return (
        ...
        "Su eventual reconocimiento depende de la confirmación del usuario SUBE Prioridad "
        "y de los filtros de seguridad posteriores."
    )


def _validation_context_notice() -> str:
    return (
        "El frontend conceptual presupone un contexto de viaje validado. "
        "El Bono Solidario no se prepara si no existe validación demostrativa de pago "
        "y contexto de transporte público en Argentina."
    )


def _privacy_notice() -> str:
    return (
        "El frontend no revela DNI, nombre, domicilio, diagnóstico, CUD, historia clínica "
        "ni certificado médico. Sólo utiliza tokens demostrativos."
    )
...
def _common_warnings() -> List[str]:
    return [
        "Frontend conceptual y demostrativo.",
        ...
        "El usuario SUBE Prioridad conserva control de la confirmación.",
        "El handoff posterior debe pasar por filtros de seguridad antifraude.",
    ]
