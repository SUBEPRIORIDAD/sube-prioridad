...
class SolidaryRejectionReason(str, Enum):
    ...
    COLLABORATOR_SELF_RECOGNITION_NOT_ALLOWED = (
        "collaborator_self_recognition_not_allowed"
    )
...
def _primary_rejection_reason(
    ...
    if (
        "priority_user_token_looks_like_free_text" in risk_flags
        or "collaborator_token_looks_like_free_text" in risk_flags
    ):
