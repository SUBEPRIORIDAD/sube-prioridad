...
La sincronización solidaria puede apoyarse en distintos canales:

    1. Celular NFC del usuario SUBE Prioridad + tarjeta SUBE del colaborador.
    2. Celular del colaborador + QR/código del usuario SUBE Prioridad.
    3. Validadora a bordo que abre una ventana lógica después del pago.
    4. Molinete o acceso de estación que abre una ventana de espera.
    5. Canal asistido, estación, terminal o tótem accesible.
    6. Confirmación diferida desde cuenta SUBE/Red SUBE/canal accesible.
    7. Registro demo para auditoría cuando no hay señal móvil.

Regla central:
    La falta de celular NO bloquea por sí sola.
    La validación Red SUBE demo puede abrir ventanas temporales lógicas.
    Cada dispositivo o contexto tiene su propia ventana.
    Las señales móviles refuerzan, pero no sustituyen el contexto de viaje.
    El sistema debe facilitar la sincronización y blindar contra fraude.
...
class SyncChannelPolicy:
    """
    Política demo de ventanas por contexto.

    La ventana no es única:
        - colectivo o validadora a bordo: corta;
        - tren con validadora dentro de la formación: corta;
        - molinete de estación/subte/tren: extendida;
        - terminal/canal asistido: extendida;
        - confirmación diferida: más amplia, pero con auditoría reforzada.
    """
...
class UserDeviceAvailability:
    """
    Disponibilidad tecnológica de un usuario.

    No implica obligación de uso.
    """
...
class RedSubeValidationContext:
    """
    Contexto demo abierto por Red SUBE, validadora o molinete.

    No representa una consulta real a Red SUBE.
    """
...
class SolidarySyncChannelRequest:
    """
    Solicitud demo para seleccionar canal de sincronización.

    Puede funcionar aunque ninguna de las partes tenga celular.
    """
...
def create_demo_sync_channel_policy(
    ...
    if in_vehicle_window_minutes <= 0:
        raise ValueError("La ventana a bordo debe ser mayor a cero.")

    if onboard_train_window_minutes <= 0:
        raise ValueError("La ventana de tren con validadora a bordo debe ser mayor a cero.")

    if station_window_minutes <= 0:
        raise ValueError("La ventana de estación debe ser mayor a cero.")

    if terminal_window_minutes <= 0:
        raise ValueError("La ventana de terminal debe ser mayor a cero.")

    if deferred_confirmation_window_hours <= 0:
        raise ValueError("La ventana diferida debe ser mayor a cero.")

    if max_context_score_for_ready_without_mobile <= 0:
        raise ValueError("El puntaje mínimo contextual debe ser mayor a cero.")

    return SyncChannelPolicy(
        in_vehicle_window_minutes=in_vehicle_window_minutes,
        onboard_train_window_minutes=onboard_train_window_minutes,
        station_window_minutes=station_window_minutes,
        terminal_window_minutes=terminal_window_minutes,
        deferred_confirmation_window_hours=deferred_confirmation_window_hours,
        require_priority_attribute_active=require_priority_attribute_active,
        require_priority_need_previously_accredited=require_priority_need_previously_accredited,
        ...
        allow_no_mobile_channels=allow_no_mobile_channels,
        allow_assisted_channel=allow_assisted_channel,
        allow_deferred_account_confirmation=allow_deferred_account_confirmation,
        max_context_score_for_ready_without_mobile=max_context_score_for_ready_without_mobile,
    )


def create_demo_user_device_availability(
    has_mobile_device: bool = True,
    has_nfc_capable_device: bool = True,
    has_app_or_account_access_now: bool = True,
    has_connectivity_now: bool = True,
    has_physical_sube_card: bool = True,
) -> UserDeviceAvailability:
    return UserDeviceAvailability(
        has_mobile_device=has_mobile_device,
        has_nfc_capable_device=has_nfc_capable_device,
        has_app_or_account_access_now=has_app_or_account_access_now,
        has_connectivity_now=has_connectivity_now,
        has_physical_sube_card=has_physical_sube_card,
    )


def create_demo_red_sube_validation_context(
...
def create_demo_solidary_sync_channel_request(
    sync_channel_event_demo_id: str = "demo-sync-channel-001",
    ...
    priority_device_availability: Optional[UserDeviceAvailability] = None,
    collaborator_device_availability: Optional[UserDeviceAvailability] = None,
    red_sube_context: Optional[RedSubeValidationContext] = None,
    confirmation_mode: ConfirmationMode = ConfirmationMode.IMMEDIATE_PRIORITY_CONFIRMATION,
    priority_user_confirms_or_can_confirm: bool = True,
    ...
    previous_handoff_security_accepted: bool = True,
    previous_proximity_guard_blocks_flow: bool = False,
) -> SolidarySyncChannelRequest:
    required_values = {
        "sync_channel_event_demo_id": sync_channel_event_demo_id,
    ...
    return SolidarySyncChannelRequest(
        sync_channel_event_demo_id=sync_channel_event_demo_id.strip(),
        ...
        priority_device_availability=priority_device_availability
        or create_demo_user_device_availability(),
        collaborator_device_availability=collaborator_device_availability
        or create_demo_user_device_availability(has_mobile_device=False, has_nfc_capable_device=False),
        red_sube_context=red_sube_context or create_demo_red_sube_validation_context(),
        confirmation_mode=confirmation_mode,
        priority_user_confirms_or_can_confirm=priority_user_confirms_or_can_confirm,
...
def create_demo_no_mobile_validator_based_request(
    created_at_utc: Optional[datetime] = None,
) -> SolidarySyncChannelRequest:
    """
    Caso central:
        usuario SUBE Prioridad sin celular
        +
        colaborador sin celular
        +
        ambos validaron en Red SUBE demo
        +
        misma validadora/unidad/servicio
        =
        canal posible por ventana contextual de validadora.
    """
    ...
    context = create_demo_red_sube_validation_context(
        ...
        vehicle_demo_id="demo-bus-vehicle-001",
    ...
    return create_demo_solidary_sync_channel_request(
        sync_channel_event_demo_id="demo-no-mobile-validator-sync-001",
        ...
        priority_user_confirms_or_can_confirm=True,
        same_vehicle_demo_id=True,
        same_validator_or_turnstile_demo_id=True,
    )


def create_demo_station_no_mobile_deferred_request(
    created_at_utc: Optional[datetime] = None,
) -> SolidarySyncChannelRequest:
    """
    Caso tren/subte:
        usuario prioritario sin celular
        +
        molinete abre ventana de estación
        +
        confirmación diferida/canal asistido
        +
        mayor auditoría.
    """
    timestamp = created_at_utc or datetime.now(timezone.utc)
    ...
    context = create_demo_red_sube_validation_context(
        ...
        validator_or_turnstile_demo_id="demo-turnstile-001",
        vehicle_demo_id=None,
        trainset_demo_id=None,
        station_demo_id="demo-station-001",
        platform_demo_id="demo-platform-001",
        priority_validation_timestamp_utc=timestamp,
        collaborator_validation_timestamp_utc=timestamp + timedelta(minutes=8),
        sync_attempt_timestamp_utc=timestamp + timedelta(minutes=45),
    )

    return create_demo_solidary_sync_channel_request(
        sync_channel_event_demo_id="demo-station-no-mobile-deferred-sync-001",
        priority_device_availability=create_demo_user_device_availability(
            has_mobile_device=False,
            has_nfc_capable_device=False,
            has_app_or_account_access_now=False,
        ...
        red_sube_context=context,
        confirmation_mode=ConfirmationMode.DEFERRED_ACCOUNT_CONFIRMATION,
        priority_user_confirms_or_can_confirm=True,
...
def evaluate_solidary_sync_channel(
    request: SolidarySyncChannelRequest,
    policy: Optional[SyncChannelPolicy] = None,
) -> SolidarySyncChannelResult:
    """
    Selecciona un canal de sincronización posible.

    No acredita beneficios.
    No calcula descuentos.
    Sólo define si puede abrirse una ventana de evaluación del Bono Solidario demo.
    """
    ...
    if hard_risk_flags:
        return _result(
            request=request,
            policy=policy,
            status=SyncChannelStatus.REJECTED,
            ...
            reason=(
                "No se abre ventana de evaluación porque existen riesgos duros "
                "o faltan condiciones mínimas de validación Red SUBE demo."
            ),
        )

    if selected_channel in {
        SyncChannel.ACCOUNT_DEFERRED_CONFIRMATION,
        SyncChannel.ASSISTED_STATION_OR_TERMINAL_CHANNEL,
    }:
        return _result(
            request=request,
            policy=policy,
            status=SyncChannelStatus.READY_DEFERRED_CONFIRMATION,
            selected_channel=selected_channel,
            window_kind=window_kind,
            window_minutes=window_minutes,
            window_matched=window_matched,
            ...
            audit_flags=audit_flags,
            reason=(
                "Se abre una ventana de evaluación con confirmación diferida o asistida. "
                "La ausencia de celular no bloquea, pero aumenta la necesidad de auditoría."
            ),
        )

    if selected_channel in {
        SyncChannel.VALIDATOR_CONTEXT_WINDOW,
        SyncChannel.TURNSTILE_OR_STATION_WINDOW,
    }:
        return _result(
            request=request,
            policy=policy,
            status=SyncChannelStatus.READY_WITHOUT_MOBILE,
            selected_channel=selected_channel,
            window_kind=window_kind,
            window_minutes=window_minutes,
            window_matched=window_matched,
            ...
            audit_flags=audit_flags,
            reason=(
                "Se abre una ventana contextual desde validadora o molinete. "
                "No se exige celular; el canal queda registrado para auditoría antifraude."
            ),
        )
    ...
    return _result(
        request=request,
        policy=policy,
        status=SyncChannelStatus.READY_REINFORCED,
        selected_channel=selected_channel,
        window_kind=window_kind,
        window_minutes=window_minutes,
        window_matched=window_matched,
        ...
        audit_flags=audit_flags,
        reason=(
            "Se seleccionó un canal de sincronización reforzado por dispositivo, "
...
def result_to_dict(result: SolidarySyncChannelResult) -> Dict[str, Any]:
    return {
        ...
        "mobile_required": result.mobile_required,
        "priority_mobile_available": result.priority_mobile_available,
        "collaborator_mobile_available": result.collaborator_mobile_available,
        "no_mobile_path_supported": result.no_mobile_path_supported,
...
def run_no_mobile_validator_demo() -> Dict[str, Any]:
...
def run_station_no_mobile_deferred_demo() -> Dict[str, Any]:
...
def _select_channel(
    request: SolidarySyncChannelRequest,
    policy: SyncChannelPolicy,
    window_matched: bool,
    context_score: int,
) -> SyncChannel:
    priority_device = request.priority_device_availability
    collaborator_device = request.collaborator_device_availability
    validation_point = request.red_sube_context.validation_point_type

    if not window_matched:
        return SyncChannel.NO_CHANNEL_AVAILABLE

    if (
        priority_device.has_mobile_device
        and priority_device.has_nfc_capable_device
        and collaborator_device.has_physical_sube_card
    ):
        return SyncChannel.PRIORITY_MOBILE_NFC_CARD_TAP

    if (
        collaborator_device.has_mobile_device
        and collaborator_device.has_app_or_account_access_now
        and collaborator_device.has_connectivity_now
    ):
        return SyncChannel.COLLABORATOR_MOBILE_QR_OR_CODE

    if policy.allow_no_mobile_channels and validation_point in {
        ValidationPointType.VEHICLE_VALIDATOR,
        ValidationPointType.ONBOARD_TRAIN_VALIDATOR,
    }:
        return SyncChannel.VALIDATOR_CONTEXT_WINDOW

    if policy.allow_no_mobile_channels and validation_point in {
        ValidationPointType.STATION_TURNSTILE,
        ValidationPointType.STATION_ACCESS_GATE,
    }:
        return SyncChannel.TURNSTILE_OR_STATION_WINDOW

    if policy.allow_assisted_channel and validation_point == ValidationPointType.TERMINAL_VALIDATOR:
        return SyncChannel.ASSISTED_STATION_OR_TERMINAL_CHANNEL

    if (
        policy.allow_deferred_account_confirmation
        and request.confirmation_mode == ConfirmationMode.DEFERRED_ACCOUNT_CONFIRMATION
        and context_score >= policy.max_context_score_for_ready_without_mobile
    ):
        return SyncChannel.ACCOUNT_DEFERRED_CONFIRMATION
...
def _hard_risk_flags(
    ...
    if _looks_like_free_text(request.priority_user_token):
        flags.append("priority_user_token_looks_like_free_text")

    if _looks_like_free_text(request.collaborator_user_token):
        flags.append("collaborator_user_token_looks_like_free_text")

    if request.priority_user_token == request.collaborator_user_token:
        flags.append("same_user_token_not_allowed")

    if policy.require_priority_attribute_active and not request.priority_user_has_active_attribute:
        flags.append("priority_attribute_not_active")

    if (
        policy.require_priority_need_previously_accredited
        and not request.priority_need_previously_accredited
    ):
        flags.append("priority_need_not_previously_accredited")

    if (
        policy.require_priority_paid_validation
        and not request.red_sube_context.priority_paid_validation_confirmed
    ):
        flags.append("priority_paid_validation_required")

    if (
        policy.require_collaborator_paid_validation
        and not request.red_sube_context.collaborator_paid_validation_confirmed
    ):
        flags.append("collaborator_paid_validation_required")

    if policy.require_priority_confirmation:
        if request.confirmation_mode == ConfirmationMode.MISSING:
            flags.append("priority_confirmation_missing")

        if not request.priority_user_confirms_or_can_confirm:
            flags.append("priority_user_cannot_confirm")
    ...
    if not request.previous_handoff_security_accepted:
        flags.append("previous_handoff_security_not_accepted")

    if request.previous_proximity_guard_blocks_flow:
        flags.append("previous_proximity_guard_blocks_flow")
...
def _audit_flags(
    ...
    if not priority_device.has_mobile_device:
        flags.append("priority_user_without_mobile_supported")

    if not collaborator_device.has_mobile_device:
        flags.append("collaborator_without_mobile_supported")

    if not priority_device.has_mobile_device and not collaborator_device.has_mobile_device:
        flags.append("no_mobile_both_users_non_blocking")
    ...
    if selected_channel in {
        ...
        flags.append("red_sube_contextual_channel_audit")

    if selected_channel == SyncChannel.VALIDATOR_CONTEXT_WINDOW:
        flags.append("validator_window_opened_demo")

    if selected_channel == SyncChannel.TURNSTILE_OR_STATION_WINDOW:
        flags.append("turnstile_or_station_window_opened_demo")

    if selected_channel == SyncChannel.ACCOUNT_DEFERRED_CONFIRMATION:
        flags.append("deferred_account_confirmation_audit")

    if request.confirmation_mode in {
        ConfirmationMode.DEFERRED_ACCOUNT_CONFIRMATION,
        ConfirmationMode.VALIDATOR_WINDOW_PENDING_CONFIRMATION,
        ConfirmationMode.ASSISTED_CONFIRMATION,
    }:
        flags.append("non_immediate_confirmation_audit")

    if window_kind in {
        WindowKind.STATION_EXTENDED_WINDOW,
        WindowKind.TERMINAL_EXTENDED_WINDOW,
        WindowKind.DEFERRED_ACCOUNT_WINDOW,
    }:
        flags.append("extended_window_audit")

    if context_score < policy.max_context_score_for_ready_without_mobile:
        flags.append("low_context_score_audit")

    return _deduplicate(flags)


def _window_kind(context: RedSubeValidationContext) -> WindowKind:
    if context.validation_point_type == ValidationPointType.VEHICLE_VALIDATOR:
        return WindowKind.IN_VEHICLE_SHORT_WINDOW

    if context.validation_point_type == ValidationPointType.ONBOARD_TRAIN_VALIDATOR:
        return WindowKind.ONBOARD_TRAIN_SHORT_WINDOW

    if context.validation_point_type in {
        ValidationPointType.STATION_TURNSTILE,
        ValidationPointType.STATION_ACCESS_GATE,
    }:
        return WindowKind.STATION_EXTENDED_WINDOW

    if context.validation_point_type == ValidationPointType.TERMINAL_VALIDATOR:
        return WindowKind.TERMINAL_EXTENDED_WINDOW
...
def _window_minutes(
    ...
    if window_kind == WindowKind.IN_VEHICLE_SHORT_WINDOW:
        return policy.in_vehicle_window_minutes

    if window_kind == WindowKind.ONBOARD_TRAIN_SHORT_WINDOW:
        return policy.onboard_train_window_minutes

    if window_kind == WindowKind.STATION_EXTENDED_WINDOW:
        return policy.station_window_minutes

    if window_kind == WindowKind.TERMINAL_EXTENDED_WINDOW:
        return policy.terminal_window_minutes

    if window_kind == WindowKind.DEFERRED_ACCOUNT_WINDOW:
        return policy.deferred_confirmation_window_hours * 60

    return policy.in_vehicle_window_minutes


def _window_matched(
    context: RedSubeValidationContext,
    window_minutes: int,
) -> bool:
    earliest = min(
        context.priority_validation_timestamp_utc,
        context.collaborator_validation_timestamp_utc,
    )
...
def _result(
    ...
    return SolidarySyncChannelResult(
        ...
        no_mobile_path_supported=policy.allow_no_mobile_channels,
        context_score=context_score,
...
def _security_summary(
    ...
    return {
        ...
        "collaborator_mobile_available": (
            request.collaborator_device_availability.has_mobile_device
        ),
        "collaborator_physical_sube_card_available": (
            request.collaborator_device_availability.has_physical_sube_card
        ),
        "priority_paid_validation_confirmed": (
            context.priority_paid_validation_confirmed
        ),
        "collaborator_paid_validation_confirmed": (
        ...
        "priority_user_confirms_or_can_confirm": (
            request.priority_user_confirms_or_can_confirm
        ),
        ...
        "same_validator_or_turnstile_demo_id": (
        ...
        "allow_deferred_account_confirmation": (
            policy.allow_deferred_account_confirmation
        ),
    }
...
def _privacy_notice() -> str:
    return (
        "El selector de canales no revela DNI, nombre, domicilio, diagnóstico, CUD, "
        "historia clínica, certificado médico, teléfono, email, IMEI, MAC, saldo "
        "ni GPS exacto. La ausencia de celular no bloquea automáticamente."
    )


def _legal_scope_notice() -> str:
    return (
        "La apertura de ventanas y canales es conceptual. No aplica descuentos reales, "
        "no actualiza tarjetas reales y no integra Red SUBE real. Una implementación "
        "requeriría autorización, integración oficial, auditoría y normativa competente."
    )


def _driver_burden_notice() -> str:
    return (
        "El chofer no selecciona canales, no verifica sincronizaciones, no decide "
        "beneficios y no administra el Bono Solidario."
    )


def _common_warnings() -> List[str]:
    return [
        "Selector conceptual y demostrativo.",
        ...
        "El usuario SUBE Prioridad puede no tener celular.",
        "El colaborador puede no tener celular.",
        "La Red SUBE demo puede abrir ventanas lógicas desde validadoras o molinetes.",
        "Cada tipo de validadora o molinete puede tener una ventana temporal distinta.",
        "La ausencia de móvil reduce refuerzo probatorio, pero no bloquea por sí sola.",
