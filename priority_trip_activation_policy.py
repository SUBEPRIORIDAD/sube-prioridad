...
def create_demo_trip_validation_context(
    ...
    return TripValidationContextDemo(
        ...
        validation_timestamp_utc=(
            validation_timestamp_utc or datetime.now(timezone.utc)
        ),
        validator_or_turnstile_demo_token=validator_or_turnstile_demo_token.strip(),
        operator_context_demo_token=(
            operator_context_demo_token.strip()
            if operator_context_demo_token
            else None
        ),
    )
...
def create_demo_trip_activation_request(
    ...
    return TripActivationRequestDemo(
        ...
        or create_demo_priority_attribute_for_trip(),
        preference=preference or create_demo_trip_activation_preference(),
        validation_context=validation_context
        or create_demo_trip_validation_context(),
    )
...
def evaluate_trip_activation_policy(
    ...
    status, scope, hint_shared, staff_only = _resolve_activation(
        request=request,
    )
...
def run_demo() -> Dict[str, Any]:
    result = evaluate_trip_activation_policy(
...
def _risk_flags(request: TripActivationRequestDemo) -> List[str]:
    ...
    if (
        preference.assistance_mode == TripAssistanceMode.LUMINOUS
        and not preference.user_allows_luminous_signal
    ):
        flags.append("luminous_signal_not_consented")

    if (
        preference.assistance_mode == TripAssistanceMode.VISIBLE
        and not preference.user_allows_public_generic_notice
    ):
        flags.append("visible_notice_not_consented")

    if (
        preference.assistance_mode == TripAssistanceMode.STATION_AUTHORIZED_STAFF
        and not preference.user_allows_authorized_staff_notice
    ):
        flags.append("authorized_staff_notice_not_consented")

    if (
        preference.user_allows_station_hint
        and not attribute.consented_operational_hint_enabled
    ):
        flags.append("station_hint_preference_without_attribute_consent")

    if (
        attribute.consented_operational_hint_enabled
        and attribute.consented_operational_hint == ConsentedOperationalHint.NONE
    ):
        flags.append("operational_hint_enabled_without_hint")

    if (
        not attribute.consented_operational_hint_enabled
        and attribute.consented_operational_hint != ConsentedOperationalHint.NONE
    ):
...
def _audit_flags(request: TripActivationRequestDemo) -> List[str]:
    ...
    if request.validation_context.transport_context in {
        TransportContext.TRAIN_STATION,
        TransportContext.SUBWAY_STATION,
        TransportContext.TURNSTILE,
        TransportContext.PLATFORM,
    }:
...
def _resolve_activation(
    request: TripActivationRequestDemo,
) -> tuple[TripActivationStatus, VisibilityScope, bool, bool]:
    preference = request.preference
    ...
    if preference.assistance_mode == TripAssistanceMode.SILENT:
        return (
            TripActivationStatus.ACTIVE_SILENT_BACKUP,
            VisibilityScope.SYSTEM_ONLY,
            False,
            False,
        )
    ...
    if preference.assistance_mode == TripAssistanceMode.INTERNAL_PASSIVE:
        return (
            TripActivationStatus.ACTIVE_PASSIVE_NOTICE,
            VisibilityScope.SYSTEM_ONLY,
            False,
            False,
        )
    ...
    if preference.assistance_mode == TripAssistanceMode.DISCREET:
        return (
            TripActivationStatus.ACTIVE_DISCREET_NOTICE,
            VisibilityScope.SYSTEM_ONLY,
            False,
            False,
        )
    ...
    if preference.assistance_mode == TripAssistanceMode.VISIBLE:
        return (
            TripActivationStatus.ACTIVE_VISIBLE_NOTICE,
            VisibilityScope.PUBLIC_GENERIC,
            False,
            False,
        )
    ...
    if (
        ...
        return (
            TripActivationStatus.ACTIVE_STATION_AUTHORIZED_NOTICE,
            VisibilityScope.AUTHORIZED_STAFF_ONLY,
            True,
            True,
        )
    ...
    return (
        TripActivationStatus.NEEDS_REVIEW,
        VisibilityScope.SYSTEM_ONLY,
        False,
        False,
    )
...
def _result(
    ...
    hint = (
        request.priority_attribute.consented_operational_hint
        if operational_hint_shared
        else ConsentedOperationalHint.NONE
    )
    ...
    return TripActivationResultDemo(
        ...
        demo_mode=DEMO_MODE,
        status=status,
        priority_attribute_active=request.priority_attribute.active,
        visible_as=visible_as,
        visibility_scope=visibility_scope,
        operational_hint_shared=operational_hint_shared,
        operational_hint=hint,
        authorized_staff_only=authorized_staff_only,
        alert_payload_demo=_alert_payload_demo(
            request=request,
            status=status,
            visibility_scope=visibility_scope,
            operational_hint_shared=operational_hint_shared,
        ...
        blocks_trip_assistance_flow=blocks_trip_assistance_flow,
...
def _alert_payload_demo(
    ...
    return {
        ...
        "status": status.value,
        "visible_as": "Usuario SUBE Prioridad"
        if request.priority_attribute.active
...
def _privacy_notice() -> str:
    return (
        "La activación por viaje no transmite DNI, nombre, diagnóstico, CUD "
        "visible, certificado médico, historia clínica, lesión, embarazo, "
        "tratamiento ni causa específica."
    )
...
def _legal_scope_notice() -> str:
    return (
        ...
        "validadores, molinetes, operadores ni bases estatales reales."
    )
...
def _common_warnings() -> List[str]:
    return [
        ...
        "El atributo activo no fuerza asistencia en todos los viajes.",
        "La visibilidad de calle es siempre genérica.",
